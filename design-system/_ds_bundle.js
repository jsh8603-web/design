/* @ds-bundle: {"format":3,"namespace":"SlidesGrabDesignSystem_646a84","components":[{"name":"SVG_NS","sourcePath":"mck/assets/chart-helpers.js"}],"sourceHashes":{"mck/assets/chart-helpers.js":"861f3d14c89e","mck/assets/donut.js":"48cf064ffbd5","mck/assets/kpi-dashboard.js":"001d70649f74","mck/assets/variance-table.js":"2048cbbe5956","mck/assets/waterfall.js":"1a8493af4090","mck/slides/deck-stage.js":"d8d952171670"},"inlinedExternals":[],"unexposedExports":[{"name":"arcPath","sourcePath":"mck/assets/chart-helpers.js"},{"name":"clear","sourcePath":"mck/assets/chart-helpers.js"},{"name":"colorVar","sourcePath":"mck/assets/chart-helpers.js"},{"name":"createSVG","sourcePath":"mck/assets/chart-helpers.js"},{"name":"el","sourcePath":"mck/assets/chart-helpers.js"},{"name":"fmt","sourcePath":"mck/assets/chart-helpers.js"},{"name":"pctFmt","sourcePath":"mck/assets/chart-helpers.js"},{"name":"polarToCart","sourcePath":"mck/assets/chart-helpers.js"},{"name":"renderDonut","sourcePath":"mck/assets/donut.js"},{"name":"renderDonutCases","sourcePath":"mck/assets/donut.js"},{"name":"renderKPIDashboard","sourcePath":"mck/assets/kpi-dashboard.js"},{"name":"renderKPIDashboardCases","sourcePath":"mck/assets/kpi-dashboard.js"},{"name":"renderVarianceTable","sourcePath":"mck/assets/variance-table.js"},{"name":"renderVarianceTableCases","sourcePath":"mck/assets/variance-table.js"},{"name":"renderWaterfall","sourcePath":"mck/assets/waterfall.js"},{"name":"renderWaterfallCases","sourcePath":"mck/assets/waterfall.js"},{"name":"resolveTarget","sourcePath":"mck/assets/chart-helpers.js"},{"name":"safeRender","sourcePath":"mck/assets/chart-helpers.js"},{"name":"snap","sourcePath":"mck/assets/chart-helpers.js"},{"name":"yoyState","sourcePath":"mck/assets/chart-helpers.js"}]} */

(() => {

const __ds_ns = (window.SlidesGrabDesignSystem_646a84 = window.SlidesGrabDesignSystem_646a84 || {});

const __ds_scope = {};

(__ds_ns.__errors = __ds_ns.__errors || []);

// mck/assets/chart-helpers.js
try { (() => {
/**
 ...
 */
function createSVG(viewWidth, viewHeight, className = "") {
  return el("svg", {
    xmlns: SVG_NS,
...
function fmt(value, {
  ...
  let sign = "";
  if (signed) {
    if (value > 0) sign = "+";else if (value < 0) sign = "\u2212"; // proper minus glyph
  } else if (value < 0) {
    sign = "\u2212";
  }
 ...
 */
function snap(value, decimals = 2) {
 ...
 */
function safeRender(selector, renderFn, ...args) {
  const tryOnce = () => {
    let node;
    try {
      node = typeof selector === "string" ? document.querySelector(selector) : selector;
    } catch {
      return false;
    }
  ...
  if (!tryOnce()) {
    ...
    document.addEventListener("slide:activated", tryOnce, {
      once: false
    });
 ...
 */
const DEFAULT_PALETTE = ["primary", "accent", "gray-2", "gray-3", "positive", "negative"];
function colorVar(name, fallbackIdx = 0) {
...
function arcPath(cx, cy, rOuter, rInner, startAngle, endAngle) {
  ...
  if (Math.abs(endAngle - startAngle) >= 359.999) {
    ...
    return [`M ${endOuter.x} ${endOuter.y}`, `A ${rOuter} ${rOuter} 0 0 1 ${midOuter.x} ${midOuter.y}`, `A ${rOuter} ${rOuter} 0 0 1 ${endOuter.x} ${endOuter.y}`, `M ${endInner.x} ${endInner.y}`, `A ${rInner} ${rInner} 0 0 0 ${midInner.x} ${midInner.y}`, `A ${rInner} ${rInner} 0 0 0 ${endInner.x} ${endInner.y}`, "Z"].join(" ");
  }
  return [`M ${endOuter.x} ${endOuter.y}`, `A ${rOuter} ${rOuter} 0 ${largeArc} 1 ${startOuter.x} ${startOuter.y}`, `L ${startInner.x} ${startInner.y}`, `A ${rInner} ${rInner} 0 ${largeArc} 0 ${endInner.x} ${endInner.y}`, "Z"].join(" ");
}
...
function pctFmt(value, {
  ...
  let sign = "";
  if (signed) {
    if (value > 0) sign = "+";else if (value < 0) sign = "\u2212";
  } else if (value < 0) {
    sign = "\u2212";
  }
 ...
 */
function yoyState(value, neutralThreshold = 0) {
  if (value == null || Number.isNaN(value)) return "neutral";
  if (value > neutralThreshold) return "positive";
  if (value < -neutralThreshold) return "negative";
  return "neutral";
}
Object.assign(__ds_scope, { SVG_NS, resolveTarget, el, createSVG, fmt, clear, snap, safeRender, colorVar, polarToCart, arcPath, pctFmt, yoyState });
})(); } catch (e) { __ds_ns.__errors.push({ path: "mck/assets/chart-helpers.js", error: String((e && e.message) || e) }); }
...
function renderDonut(target, data) {
  const container = __ds_scope.resolveTarget(target);
  __ds_scope.clear(container);
  ...
  const arcs = segs.map((s, i) => {
    ...
    return {
      ...
      colorRef: __ds_scope.colorVar(s.color, i)
    };
  });
  ...
  // ── 4. SVG root + segments ─────────────────────────────────────────────
  const svg = __ds_scope.createSVG(VW, VH, "donut-chart");

  // Track ring background (light gray) — visible if a single 100% segment is drawn
  svg.appendChild(__ds_scope.el("circle", {
    class: "donut-track",
    cx: donutCx,
    cy: donutCy,
    r: (rOuter + rInner) / 2,
  ...
  // Arcs
  arcs.forEach(a => {
    const d = __ds_scope.arcPath(donutCx, donutCy, rOuter, rInner, a.startAngle, a.endAngle);
    const seg = __ds_scope.el("path", {
  ...
  if (data.center_value) {
    svg.appendChild(__ds_scope.el("text", {
  ...
  arcs.forEach((a, i) => {
    ...
    svg.appendChild(__ds_scope.el("rect", {
      class: "donut-swatch",
      x: swatchX,
      y: rowY - 7,
      width: 14,
 ...
 */
function renderDonutCases(target, cases) {
  const container = __ds_scope.resolveTarget(target);
  __ds_scope.clear(container);
  cases.forEach(c => {
...
Object.assign(__ds_scope, { renderDonut, renderDonutCases });
})(); } catch (e) { __ds_ns.__errors.push({ path: "mck/assets/donut.js", error: String((e && e.message) || e) }); }

// mck/assets/kpi-dashboard.js
try { (() => {
/**
 ...
 */
function renderKPIDashboard(target, data) {
  const container = __ds_scope.resolveTarget(target);
  __ds_scope.clear(container);
  ...
  if (layout === "auto") {
    if (n === 3) layout = "1x3";else if (n === 4) layout = "2x2";else if (n === 5) layout = "1x5";else if (n === 6) layout = "2x3";else if (n === 8) layout = "2x4";else layout = `1x${n}`; // fallback: single row
  }
  ...
  kpis.forEach(kpi => {
    ...
    if (kpi.yoy != null) {
      ...
      const state = __ds_scope.yoyState(kpi.yoy, neutralThreshold);
      ...
      yoy.textContent = `${marker} ${__ds_scope.pctFmt(kpi.yoy, {
        unit: yoyUnit
      })}`;
 ...
 */
function renderKPIDashboardCases(target, cases) {
  const container = __ds_scope.resolveTarget(target);
  __ds_scope.clear(container);
  cases.forEach(c => {
    const wrap = document.createElement("div");
...
Object.assign(__ds_scope, { renderKPIDashboard, renderKPIDashboardCases });
})(); } catch (e) { __ds_ns.__errors.push({ path: "mck/assets/kpi-dashboard.js", error: String((e && e.message) || e) }); }

// mck/assets/variance-table.js
try { (() => {
/**
 ...
 */
function renderVarianceTable(target, data) {
  const container = __ds_scope.resolveTarget(target);
  __ds_scope.clear(container);
  ...
  const rows = items.map((it, idx) => {
    ...
    if (pct != null) {
      ...
      if (effective > neutral) state = "positive";else if (effective < -neutral) state = "negative";
    }
    return {
      ...it,
      idx,
    ...
    columns.forEach(col => {
      ...
      switch (col) {
        ...
        case "actual":
          ...
          break;
        case "variance_abs":
          content = __ds_scope.fmt(row.abs, {
            signed: true,
            unit: row.unit
          });
          ...
          break;
        case "variance_pct":
          content = row.pct == null ? "—" : __ds_scope.pctFmt(row.pct);
          if (row.state !== "neutral") {
            td.classList.add(row.state === "positive" ? "var-positive" : "var-negative");
          } else {
            td.classList.add("var-neutral");
...
function formatVal(v, unit) {
  ...
  const num = decimals === 0 ? Math.round(abs).toLocaleString("en-US") : abs.toFixed(decimals);
  const sign = v < 0 ? "\u2212" : "";
 ...
 */
function renderVarianceTableCases(target, cases) {
  const container = __ds_scope.resolveTarget(target);
  __ds_scope.clear(container);
  cases.forEach(c => {
    const wrap = document.createElement("div");
...
Object.assign(__ds_scope, { renderVarianceTable, renderVarianceTableCases });
})(); } catch (e) { __ds_ns.__errors.push({ path: "mck/assets/variance-table.js", error: String((e && e.message) || e) }); }

// mck/assets/waterfall.js
try { (() => {
/**
 ...
 */
function renderWaterfall(target, data) {
  const container = __ds_scope.resolveTarget(target);
  __ds_scope.clear(container);
  const items = data.items || [];
  if (items.length < 2) {
  ...
  const yFor = v => __ds_scope.snap(baseline - v / maxVal * plotH);

  // ── 4. Build SVG ───────────────────────────────────────────────────────
  const svg = __ds_scope.createSVG(VW, VH, "waterfall-chart");

  // 4a. Baseline rule
  svg.appendChild(__ds_scope.el("line", {
    class: "baseline",
    x1: padLeft,
    y1: baseline,
    x2: padLeft + plotW,
  ...
  if (showConnector) {
    for (let i = 0; i < bars.length - 1; i++) {
      ...
      const yJoin = next.type === "base" || next.type === "subtotal" ? yFor(cur.runningAfter) : yFor(cur.runningAfter);
      const xFrom = __ds_scope.snap(xCenter(i) + barW / 2);
      const xTo = __ds_scope.snap(xCenter(i + 1) - barW / 2);
      svg.appendChild(__ds_scope.el("line", {
        class: "connector",
        x1: xFrom,
        y1: yJoin,
  ...
  // 4c. Bars + labels
  bars.forEach((b, i) => {
    const x = __ds_scope.snap(xCenter(i) - barW / 2);
    ...
    const group = __ds_scope.el("g", {
      class: klass,
      "data-index": i
    });

    // rect
    group.appendChild(__ds_scope.el("rect", {
      x: x,
      y: yTop,
      width: __ds_scope.snap(barW),
      height: __ds_scope.snap(h)
    }));

    // value label above the bar
    const valueText = b.type === "base" || b.type === "subtotal" ? __ds_scope.fmt(b.value, {
      unit: i === 0 ? unit : ""
    }) // only first base shows unit
    : __ds_scope.fmt(b.signedDelta, {
      signed: true
    });
    group.appendChild(__ds_scope.el("text", {
      class: "bar-value",
      x: __ds_scope.snap(xCenter(i)),
      y: __ds_scope.snap(yTop - 8)
    }, valueText));
    ...
    labelLines.forEach((line, li) => {
      group.appendChild(__ds_scope.el("text", {
        class: "bar-label",
        x: __ds_scope.snap(xCenter(i)),
        y: __ds_scope.snap(labelY0 + li * lineH)
      }, line.trim()));
    });
  ...
  if (showNet && bars.length >= 2) {
    ...
    if (firstBase && lastBase && firstBase !== lastBase) {
      ...
      const txt = pct == null ? `\uc21c\ubcc0\ub3d9 ${__ds_scope.fmt(net, {
        signed: true,
        unit
      })}` : `\uc21c\ubcc0\ub3d9 ${__ds_scope.fmt(net, {
        signed: true,
        unit
 ...
 */
function renderWaterfallCases(target, cases) {
  const container = __ds_scope.resolveTarget(target);
  __ds_scope.clear(container);
  cases.forEach(c => {
    const wrap = document.createElement("div");
...
Object.assign(__ds_scope, { renderWaterfall, renderWaterfallCases });
})(); } catch (e) { __ds_ns.__errors.push({ path: "mck/assets/waterfall.js", error: String((e && e.message) || e) }); }

// mck/slides/deck-stage.js
try { (() => {
/**
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "mck/slides/deck-stage.js", error: String((e && e.message) || e) }); }

__ds_ns.SVG_NS = __ds_scope.SVG_NS;
