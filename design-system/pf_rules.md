...
- 슬라이드 HTML 파일명 불일치 → 반드시 `slide-NN.html` (NN은 01부터 시작하는 두 자리 숫자) 형식 준수 [PF-68 *(미구현/계획 — 코드에 PF-68 게이트 없음)*]
...
## 금지 — 텍스트 컨테이너 세로 넘침 [PF-28]

- PF-28 코드 게이트(`scripts/preflight-html.js` PF-28 computed, --full): **텍스트 컨테이너 scrollHeight 가 clientHeight 를 초과**(세로 넘침)하면 ERROR. 단어 수 한도는 코드 게이트로 강제하지 않는다.
- 정적 word-equiv 밀도 게이트(>80 WARN / >120 ERROR)는 표/카드 셀을 과대계수해 전수 FP 라 **비활성**(2026-06-15) — 실측 scrollHeight 넘침으로 대체.

### 비강제 작성 규약 — 슬라이드당 단어 수 (6x6 / 5x5)

- 권장: 슬라이드당 **영문 120단어 / CJK 80단어 이하** (출처·캡션 제외). 이는 작성 가이드일 뿐 **코드 게이트로 강제되지 않음**.
...
## 필수 — 이미지 최소 크기 (슬라이드 면적 1/4) [IL-63 / PF-70]

- **핵심 원칙**: 콘텐츠 이미지(`assets/`)는 슬라이드 면적(720×405=291,600 sq pt)의 **최소 1/4(≈25%)**을 차지해야 한다. (25%/260/180 게이트 = PF-70 ERROR. PF-57 은 width<100pt / height<80pt "컨테이너 대비 과소" WARN 으로 별개 룰)
...
- **비율 왜곡 절대 금지**: `<img>` 태그에 width/height를 지정할 때, 원본 이미지의 종횡비(aspect ratio)를 변경하는 조합 금지 → PF-21 **WARN**
- **PF-21 코드 게이트 동작**(`scripts/preflight-html.js` --full): distorted 판정 = 표시 박스의 **scale 축 차이 > 5%** 일 때만 WARN. `object-fit: cover/contain/scale-down` 은 브라우저가 비율을 강제 보존하므로 **면제**(2026-06-15, AR 보존시 렌더 왜곡 0). lowres(>2배 확대)·lowdpi(<72)도 모두 WARN.
...
> **PF-60 현재 비활성**(2026-06-15, `scripts/preflight-html.js` 에서 `return []` 처리 — 배지 fill 은 PPTX서 정상 렌더되고 부모배경 오귀속 FP). 같은 의도(흰on흰 등 저대비)는 실측 **PF-71(--full WCAG 대비)**·**PF-24(비활성)** 가 아니라 PF-71 이 배지 텍스트까지 대체 커버. 아래는 디자인 작성 규약으로 유지.
...
## 필수 — CJK 카드 텍스트 [IL-06 / PF-08 → PF-23]

> **PF-08 폐지**: 코드(`scripts/preflight-html.js` L1572)에서 PF-08 은 **REMOVED — PF-23 에 흡수**. PF-23(CJK 텍스트 밀도, --full scrollWidth 초과 정밀 예측)이 PF-08 발화 슬라이드를 전부 더 정확히 커버. 아래 작성 규약은 PF-23 게이트로 검증된다.

- 카드 내 CJK font-size ≤ 11pt (디자인 권장 — 코드 강제는 PF-23 의 실측 overflow)
...
**10pt = 디자인 권장 floor** (특히 Badge, Footer 등 부수적 요소에서도 9pt 사용 지양). 콘텐츠 초과 시 폰트 축소 대신 슬라이드 분할.

> **코드 게이트 주의**: PF-25 의 실측 차단 게이트(`scripts/preflight-html.js` PF-25 computed, `pt >= 6.99 continue`)는 **본문 영역 <7pt 만 ERROR** 다 (FP 회피 보수화 — CSS 7pt→6.9997pt 환산오차 통과, 차트라벨/표/svg 등 aux 역할은 제외). 즉 8~9pt 본문은 게이트 통과한다. 10pt 권장 floor 는 디자인 규약, <7pt 본문은 자동 차단 — 두 기준 구분.
...
> **box-shadow 주의**: 일반(외곽) `box-shadow` 는 PPTX 변환 시 무시되지만 **전용 PF 룰로 자동 검출되지 않는다**(코드엔 `inset` 만 PF-22 WARN). 디자인 의도상 슬라이드에서는 box-shadow 미사용 권장(elevation 은 tint/border 로 — `domain/fpna_invariants.md` §4.3). PF-66 은 box-shadow 가 아니라 **overflow:hidden 콘텐츠 클립** 게이트다(아래 §PF-66 참조).

---

## v3 통합 신규 룰 (PF-69 ~ PF-75) — mck 흡수

slides-grab Design System v3 통합으로 추가된 7 룰. **모두 현재 `scripts/preflight-html.js` 에 미구현(계획 단계)** — 코드엔 PF-67/68/69 게이트가 없고, 아래 PF-70~73 은 코드의 PF-70(이미지 품질)·PF-71(WCAG 대비)과 번호만 충돌하는 별개 문서 구조 게이트다(L287 주의 참조). 구현 전까지 작성 규약·설계 의도로만 적용.
