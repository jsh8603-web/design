> 🚫 **사내 미사용 문서 (원본 slides-grab)**: 이 문서가 다루는 단계/기능 전체가 사내 불가(Gemini 이미지 생성 / Gemini Vision)다. **참고 기록용**으로만 보존하며 사내 파이프라인에서는 실행하지 않는다. 이미지는 차트/표/도형 우선 + 수동 배치(`design-system/image_slot_contract.md`).
...
체크리스트 구조: `CLAUDE.md` §공통 절차 참조 (오탐/정탐-수정 → A~I, 정탐-한계 → 간소화 A~C).
...
3. **정탐-한계** → CLAUDE.md §공통 절차 간소화 A~C 체크리스트 생성 + 실행
