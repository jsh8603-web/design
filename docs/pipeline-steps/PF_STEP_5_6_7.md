...
디버깅/신규 패턴 참조 시에만 `pptx-inspection-log.md` 참조.
...
2. 재변환 (최대 3회)
3. **수정 내용을 `pptx-inspection-log.md`에 기록** (이슈 패턴 섹션에 추가)
...
4. **이슈 확정 시 자동 수정**
   ...
   - 수정 내용을 `pptx-inspection-log.md`에 기록
...
## Step 7.6 — 3분류 판정 검증 (프로덕션 후 필수 — 건너뛰기 금지)

프로덕션 중 수행한 모든 3분류 판정(오탐/정탐-수정/정탐-한계)이 CLAUDE.md §판정별 수정 대상 테이블의 B·C·D 규칙을 **실제로 충족했는지** 사후 검증한다.

**트리거**: Step 7.5 완료 직후, 완료 보고 전. progress.md에 3분류 판정 이슈가 1건 이상 존재하면 실행.

**절차**:
1. progress.md의 모든 이슈(#1~#N)를 읽고, PF/VP/COM 관련 판정을 추출
2. 각 **정탐-수정** 이슈에 대해 검증:
   - B(대상 수정): HTML/PPTX 결과물을 실제로 수정했는가?
   - **C(원인 수정)**: §판정별 수정 대상 C열 파일(`html-prevention-rules.md`, `html2pptx.cjs`)을 **실제로 수정했는가?** — HTML만 수정하고 생성 규칙 미수정은 **위반**
   - **D(재발 방지)**: D열 파일(`preflight-html.js`, `validate-pptx.js`)을 수정했거나 기존 규칙 코드(PF-XX, VP-XX)를 **명시**했는가?
3. 각 **정탐-한계** 이슈에 대해 검증:
   - B(생성 금지 규칙 등록): `html-prevention-rules.md`에 "이 패턴 사용 금지/주의" 규칙을 등록했는가? — ERROR인데 미등록은 **위반**
   - C(심각도 재검토): ERROR 유지/WARN 강등 판단을 명시했는가?
4. 각 **오탐** 이슈에 대해 검증:
   - C(원인 수정): 탐지 코드를 실제로 수정했는가?
5. 위반 발견 시: 즉시 보완 (생성 규칙 추가, 변경 로그 기록, 테스트)
6. **보완 후 회귀 테스트** 실행: `node tests/run-full-regression.mjs` (탐지 규칙 수정 시 필수)

**완료 게이트**: 위반 0건 + 보완 테스트 PASS 후에만 완료 보고 가능.
