...
NanoBanana: `node scripts/generate-images.mjs --outline outline.md --output assets/` (가이드: `nanoBanana-guide.md`)
...
**체크리스트 기재 형식**: B/C/D 각 항목은 **수정한 파일명 + 변경 내용**을 기재해야 한다. "완료"만 적는 것은 금지.
- ✅ `C. 원인 수정: html-prevention-rules.md에 "PF-07: <p>에 background/border 금지" 규칙 추가`
- ✅ `D. 재발 방지: preflight-html.js checkPF07 severity fmtWarn→fmtError 변경`
- ❌ `C. 원인 수정: 완료` ← 검증 불가, 금지
...
### progress.md `## 현재 상태` 필수 필드

progress.md 최초 생성 시 `## 현재 상태` 섹션에 아래 필드를 기재:
```
## 현재 상태
Phase: 진행중  |  시작: YYYY-MM-DD  |  종료: (미정)
Agent: Claude Opus 4.6
```
- `Phase`: 진행중 → 완료 (SUCCESS 출력 직전 갱신)
- `종료`: 완료 시 실제 날짜로 갱신
- `Agent`: 작업을 수행하는 AI 에이전트명. Gemini/Claude 결과물 구분 및 품질 추적에 사용
...
### 3계층 Tier 오버라이드
이 프로젝트는 `.claude/.tier-config`로 **Tier 3**을 선언한다. 기본 단일 agent(Tier 1)에서 비활성화되는 CV/Knowledge/Quality hook이 이 프로젝트에서는 활성 상태. `exclude` 배열로 프로젝트 자체 guard와 중복되는 hook(pipe.progress-inject, pipe.checklist-prior, gemini.sync-check)은 제외.
...
**전역 `pipe.issue-gate` 양보**: 전역 가드는 `.claude/settings.local.json`에 `checklist-guard`가 등록된 프로젝트에서는 `pipe.issue-gate`를 skip한다. 프로젝트 로컬 가드가 더 세밀하게 처리하기 때문.

## 프로젝트 로컬 훅/가드 체계

`.claude/settings.local.json`에 등록된 프로젝트 전용 훅/가드. 전역 레지스트리(`~/.claude/docs/operations/hook-guard-review.md`)에 포인터 연결됨.

### PreToolUse 훅

| ID | 스크립트 | Matcher | 역할 |
|---|---------|---------|------|
| **sg.session-restore** | `session-restore-guard.mjs` | Edit\|Write\|Bash | progress.md 존재 시 `.restore-marker` mtime 비교 → stale이면 차단, 세션 복원 강제 |
| **sg.checklist-guard** | `checklist-guard.mjs` | Edit\|Write | 9개 규칙으로 파이프라인 코드/슬라이드 수정을 체크리스트 기반 차단 |

### PostCompact 훅

| ID | 스크립트 | 역할 |
|---|---------|------|
| **sg.post-compact** | `post-compact-restore.mjs` | `.restore-marker` 삭제 → 다음 Edit/Write 시 세션 복원 강제 |

### 파이프라인 내장 가드

| ID | 스크립트 | 역할 |
|---|---------|------|
| **sg.auto-checklist** | `auto-checklist.mjs` | 파이프라인 ERROR 시 progress.md에 placeholder 체크리스트 자동 주입 |

### checklist-guard 9개 규칙 상세

| Rule | 조건 | 대상 | 동작 |
|------|------|------|------|
| 1 | 체크리스트 없이 파이프라인 코드 수정 | PIPELINE_FILES (~31개) | BLOCK |
| 2 | 미완료 체크리스트 있는데 Step 진행 | progress.md | WARN |
| 3a/3b | 3분류 판정 없이/미완료 시 슬라이드 수정 | slides/*.html | BLOCK |
| 4 | Step 7 완료 + V-NN 미검증 | 모든 파일 (progress 제외) | BLOCK |
| 4b | 스트레스/회귀 테스트 부적절 이월 | 동일 | WARN |
| 5 | Step 7 완료 + 활성 규칙 미체크 | 동일 | BLOCK |
| 6 | Write 덮어쓰기 시 필수 섹션 삭제 | CLAUDE.md, rules/ 등 | BLOCK |
| 7a | 미완료 정탐-수정/오탐 이슈 → 다른 콘텐츠 수정 | slides/ 콘텐츠 | BLOCK |
| 7b | 정탐-수정/오탐 완료 시 필수 항목 누락 | progress.md Write | BLOCK |
...
- `research-supplement-rules.md` — 소스 커버리지 확인 + 내용 보완 리서치 의무 (Step 0.5~4)

## 폴더 구조

- `slides/{프레젠테이션명}/` — 슬라이드 HTML + 출력 PPTX/PDF (프로덕션 전용). **PPTX/PDF 및 관련 산출물은 반드시 해당 프레젠테이션 폴더에 직접 저장한다** (output/ 등 하위 폴더 생성 금지)
...
## Multi-Agent (Gemini) 연동

Gemini CLI를 Claude와 동등한 코딩 에이전트로 운용. 동일 장소에 동일 방식으로 기록.

> **상세**: `.claude/docs/gemini-multi-agent-setup.md`
> **실행 계획**: `plan-gemini-architecture.md`

### 기술 제약
Gemini CLI는 `.claude/` 경로를 workspace 격리로 차단 (ReadFile/WriteFile 모두). NTFS Junction도 원본 경로 resolve하여 차단. **루트 파일과 CLAUDE.md만 접근 가능.**
단, shell 명령(`node scripts/...`)은 Node.js 프로세스라 제한 없음 → **테스트/회귀분석 전부 실행 가능.**

### 해결 방법 3가지
| 방법 | 대상 | 구현 |
|------|------|------|
| **A. Write-Proxy** | `~/.claude/` 전용 파일 (promotion-log, memory) | `.gemini-outbox/` → hook이 `~/.claude/`로 자동 복사 |
| **B. 루트 복사본** | `.claude/docs/` 읽기 전용 참조 | `UPPER_CASE.md`로 루트에 복사 |
| **C. 루트 단일 원본** | Claude+Gemini **모두 수정**하는 파일 | `.claude/docs/`에서 루트로 이동 (이중 관리 방지) |

### 루트 이동 대상 (방법 C, 공유 수정 파일)
| 파일 | 수정 시나리오 |
|------|-------------|
| `html-prevention-rules.md` | 정탐-수정/한계 시 생성 규칙 수정 |
| `pptx-inspection-log.md` | 정탐-한계 시 IL 기록 |
| `nanoBanana-guide.md` | 이미지 정탐-수정/한계 시 생성 규칙 수정 |

**판단 기준**: Claude만 수정 → `.claude/docs/` 유지. 양쪽 수정 → 루트 단일 원본. `~/.claude/` 전용 → Write-Proxy.

### 보호 체계
- `claude-guards.toml`: CLAUDE.md, GEMINI.md(전역+프로젝트) 쓰기 차단 → 규칙 복제는 Claude/사용자만
- 기존 보호: `~/.claude/rules/`, `hooks/`, `skills/`, `keys/` 쓰기 차단 유지

### 프로젝트 GEMINI.md 역할
CLAUDE.md 전체를 인라인 복제하지 않고, **읽기 경로 매핑 + 기록 장소 + 코드 수정 대상 + 전역 예외** (~70줄)만 기술.
