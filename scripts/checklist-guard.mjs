 ...
 * 7a. BLOCK slide HTML edits on other slides when 정탐-수정/오탐 issue has open items
 * 7b. BLOCK progress.md Write closing 정탐-수정/오탐 without mandatory items
 *
...
const _EARLY_LOG = join(homedir(), '.claude', 'hook-debug-early.log');
try { appendFileSync(_EARLY_LOG, `${new Date().toISOString()} [checklist-guard] START cwd=${process.cwd()} pid=${process.pid}\n`); } catch(e) {}
...
/**
 * Parse issue sections with detailed info for Rule 7.
 * Returns: [{ title, verdict, items: [{letter, checked, text}], slides: Set, isExpanded }]
 */
function parseIssueDetails(content) {
  const lines = content.split('\n');
  const issues = [];
  let current = null;
  const ISSUE_PATTERN = /^### (이슈|사용자 피드백|정탐-한계|정탐-수정|규칙 개선|승격)/;
  ...
  for (const line of lines) {
    if (ISSUE_PATTERN.test(line)) {
      flush();
      // Extract slide numbers from title (e.g., "slide-01", "slide-03")
      const slideMatches = line.match(/slide-(\d+)/g) || [];
      const slides = new Set(slideMatches.map(s => s.toLowerCase()));
      current = { title: line.trim(), verdict: null, items: [], slides, isExpanded: false };
      continue;
    }
    if (/^##[^#]/.test(line) || (/^### /.test(line) && !ISSUE_PATTERN.test(line))) {
      flush();
      current = null;
      continue;
    }
    ...
    const trimmed = line.trim();
    // Extract verdict from A. 판정 line
    if (/A\.\s*판정/.test(trimmed)) {
      if (/정탐-수정/.test(trimmed)) current.verdict = '정탐-수정';
      else if (/오탐/.test(trimmed)) current.verdict = '오탐';
      else if (/정탐-한계/.test(trimmed)) current.verdict = '정탐-한계';
      // Check if it's a placeholder (미입력)
      if (/미입력/.test(trimmed)) current.verdict = null;
    }
    // Parse checklist items
    const itemMatch = trimmed.match(/^- \[([ x])\]\s*([A-I])\.\s*(.*)/);
    if (itemMatch) {
      current.items.push({
        letter: itemMatch[2],
      ...
      // If we see B~I items that are not just placeholders, it's expanded
      if (itemMatch[2] !== 'A' && !/미입력|placeholder/i.test(itemMatch[3])) {
...
/**
 * Check if a file is any presentation content file in slides/{pres}/
 * (excludes progress.md and change-log.md which are tracking files)
 */
function isPresentationContent(relPath) {
  if (!/^slides\/[^/]+\//.test(relPath)) return false;
  const filename = relPath.split('/').pop();
  if (/^(progress|change-log)\.md$/.test(filename)) return false;
  return true;
}
...
3. 차단 해소: 위 분석 완료 후, 규칙에 따라 올바른 선행 작업을 수행한 뒤 재시도`;
  ...
  // Claude Code PreToolUse hook: exit(2) = deny, stderr = reason, stdout JSON = additionalContext
...
async function main() {
  ...
  const isContent = isPresentationContent(relPath);
  const isProgressMd = /slides\/[^/]+\/progress\.md$/.test(relPath);

  // Not a monitored file → allow
  if (!isPipelineFile && !isContent && !isProgressMd) {
    process.exit(0);
  }
  ...
  const progressPath = isContent
    ? findProgressForSlide(relPath)
    : findActiveProgress();

  if (!progressPath) {
    if (isPipelineFile || isContent) {
...
tests/stress-slides/에 TP/FP 테스트 슬라이드를 생성하여 PF/VP를 실행하세요.`;
  ...
  // --- Rule 7b: 정탐-수정/오탐 이슈 완료 시 필수 항목 검증 ---
  if (isProgressMd && toolName === 'Write') {
    const newContent = toolInput.content || '';
    const issues = parseIssueDetails(newContent);
    for (const issue of issues) {
      if (issue.verdict !== '정탐-수정' && issue.verdict !== '오탐') continue;
      if (!issue.isExpanded) continue;
      // Check if all items are checked
      const allChecked = issue.items.length > 0 && issue.items.every(i => i.checked);
      if (!allChecked) continue;
      // Validate mandatory keywords in item texts
      const allText = issue.items.map(i => i.text).join(' ');
      const docsOnly = /N\/A|문서|docs-only/i.test(allText);
      const hasCodeFix = /탐지|규칙|코드|수정|detection|prevention|preflight|validate|generate-images|PF-|VP-|IP-|IV-|IC-|VQA/i.test(allText);
      const hasChangeLog = /change-log|C-\d+/i.test(allText);
      const hasTest = /테스트|PASS|test|스트레스|stress/i.test(allText);
      const hasRegression = /회귀|regression/i.test(allText);
      const missing = [];
      if (!docsOnly && !hasCodeFix) missing.push('코드/탐지 수정 항목');
      if (!docsOnly && !hasChangeLog) missing.push('change-log 기록');
      if (!hasTest) missing.push('테스트 실행/통과');
      if (!hasRegression) missing.push('회귀 테스트');

      // Positive-match file patterns for each pipeline
      const DETECTION_FILES = /generate-images|preflight-html|validate-pptx|validate-pptx-com/i;
      const GENERATION_FILES = /html-prevention-rules|nanoBanana-guide|design-skill|html2pptx|generate-images|직접\s*수정|수동\s*생성/i;
      const RULE_CODE_PATTERN = /PF-\d+|VP-\d+|IP-\d+|IV-\d+|IC-\d+|VQA|COM/i;

      // Rule 7b-2~3: 오탐 — C와 D 모두 탐지코드 파일명 필수 (포지티브 검증)
      if (issue.verdict === '오탐') {
        const cItem = issue.items.find(i => i.letter === 'C');
        const dItem = issue.items.find(i => i.letter === 'D');
        // C: 탐지코드 파일명 필수
        if (cItem && cItem.checked && !DETECTION_FILES.test(cItem.text)) {
          missing.push('오탐 C(원인 수정)에 탐지코드 파일명 필수 — generate-images.mjs/preflight-html.js/validate-pptx.js 중 수정 대상 명시');
        }
        // D: 탐지코드 파일명 또는 "C=D" 필수
        if (dItem && dItem.checked && !DETECTION_FILES.test(dItem.text) && !/C\s*=\s*D/i.test(dItem.text)) {
          missing.push('오탐 D(재발 방지)에 탐지코드 파일명 또는 "C=D" 필수');
        }
      }

      // Rule 7b-4~5: 정탐-수정 — C는 생성규칙 파일명, D는 탐지코드 파일명/규칙코드 필수
      if (issue.verdict === '정탐-수정') {
        const cItem = issue.items.find(i => i.letter === 'C');
        const dItem = issue.items.find(i => i.letter === 'D');
        // C: 생성 규칙 파일명 또는 "직접 수정/수동 생성" 필수
        if (cItem && cItem.checked && !GENERATION_FILES.test(cItem.text)) {
          missing.push('정탐-수정 C(원인 수정)에 생성 규칙 파일명 필수 — html-prevention-rules.md/nanoBanana-guide.md/design-skill 또는 "직접 수정" 명시');
        }
        // D: 탐지코드 파일명 또는 규칙 코드(PF-XX/VP-XX 등) 필수
        if (dItem && dItem.checked && !DETECTION_FILES.test(dItem.text) && !RULE_CODE_PATTERN.test(dItem.text)) {
          missing.push('정탐-수정 D(재발 방지)에 탐지코드 파일명 또는 규칙 코드(PF-XX/VP-XX 등) 필수');
        }
      }

      if (missing.length > 0) {
        block('7b', `정탐-수정/오탐 이슈 완료 시 필수 항목 누락 — ${issue.title}`, {
          relPath, progressPath,
          violationDetails: `이슈 "${issue.title}"이 전부 [x]로 닫히려 하나 필수 항목 누락:\n${missing.map(m => '  - ' + m).join('\n')}\n\n정탐-수정/오탐 판정 시 원인 수정 + 재발 방지 + change-log + 테스트가 완료되어야 이슈를 닫을 수 있습니다.\n오탐: C=D (탐지 코드 파일을 직접 수정). 정탐-수정: C는 생성 규칙, D는 탐지 코드.`,
          ruleFiles: ['CLAUDE.md §공통 절차', 'CLAUDE.md §코드 변동 로그', 'CLAUDE.md §판정별 수정 대상 파일']
        });
      }
    }
  }
  ...
  // --- Rule 7a: 미완료 정탐-수정/오탐 이슈 → 다른 콘텐츠 수정 차단 ---
  // Covers all presentation content: slide HTML, images, outlines, assets, etc.
  if (isContent) {
    const issues7a = parseIssueDetails(content);
    // Extract file stem from relPath (e.g., "slide-03.html" → "slide-03", "assets/img-01.png" → "img-01")
    const editingFileStem = relPath.split('/').pop().replace(/\.[^.]+$/, '').toLowerCase();
    // Collect all open issues with slides info
    const openIssues = issues7a.filter(issue => {
      if (issue.verdict !== '정탐-수정' && issue.verdict !== '오탐') return false;
      if (!issue.isExpanded) return false;
      if (issue.slides.size === 0) return false;
      const openItems = issue.items.filter(i => !i.checked);
      return openItems.length > 0;
    });
    if (openIssues.length > 0) {
      // Allow if editing file matches ANY open issue's slides (prefix match)
      const matchesAny = openIssues.some(issue =>
        [...issue.slides].some(s => editingFileStem === s || editingFileStem.startsWith(s + '-'))
      );
      if (!matchesAny) {
        const firstIssue = openIssues[0];
        const openItems = firstIssue.items.filter(i => !i.checked);
        block('7a', `미완료 정탐-수정/오탐 이슈 존재 — 다른 콘텐츠 수정 차단 (${relPath})`, {
          relPath, progressPath,
          violationDetails: `이슈 "${firstIssue.title}" 외 ${openIssues.length - 1}건에 미완료 항목이 남아있습니다.\n이슈 관련 슬라이드: ${openIssues.flatMap(i => [...i.slides]).join(', ') || '(없음)'}\n편집 대상: ${editingFileStem}\n\n정탐-수정/오탐 이슈의 모든 항목(원인 수정, 재발 방지, change-log, 테스트)을 완료한 후 다른 콘텐츠로 이동하세요.`,
          ruleFiles: ['CLAUDE.md §공통 절차 — 완료 게이트']
        });
      }
    }
  }
