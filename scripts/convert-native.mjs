...
/**
 * Patch notesSlide XMLs: add wrap="square" and placeholder dimensions.
 * PptxGenJS addNotes() omits these, causing single-line display in PowerPoint.
 */
async function patchNotesSlides(pptxPath) {
  const data = fs.readFileSync(pptxPath);
  const zip = await JSZip.loadAsync(data);
  const noteFiles = Object.keys(zip.files).filter(f =>
    /notesSlide\d+\.xml$/.test(f) && !f.includes('_rels')
  );
  if (noteFiles.length === 0) return;

  for (const f of noteFiles) {
    let xml = await zip.file(f).async('text');

    // Add spPr dimensions to Notes body placeholder (type="body")
    xml = xml.replace(
      /(<p:ph type="body" idx="1"\/>[\s\S]*?<\/p:nvSpPr>)<p:spPr\/>/,
      '$1<p:spPr><a:xfrm><a:off x="685800" y="4714875"/>' +
      '<a:ext cx="5486400" cy="3086100"/></a:xfrm></p:spPr>'
    );

    // Add wrap="square" to bodyPr of notes body
    xml = xml.replace(
      /(<p:ph type="body" idx="1"[\s\S]*?<p:txBody>)<a:bodyPr\/>/,
      '$1<a:bodyPr wrap="square" rtlCol="0"/>'
    );

    // Add spPr dimensions to Slide Image placeholder
    xml = xml.replace(
      /(<p:ph type="sldImg"\/>[\s\S]*?<\/p:nvSpPr>)<p:spPr\/>/,
      '$1<p:spPr><a:xfrm><a:off x="685800" y="514350"/>' +
      '<a:ext cx="5486400" cy="3086100"/></a:xfrm></p:spPr>'
    );
  ...
  console.log(`  Patched ${noteFiles.length} notesSlides (wrap + dimensions)`);
}
...
async function main() {
  ...
  // await patchNotesSlides(outputPath);
