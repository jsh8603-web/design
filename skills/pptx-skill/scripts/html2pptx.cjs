    ...
    if (el.type === 'image') {
      if (el.src.startsWith('data:')) {
        targetSlide.addImage({
          ...
          w: el.position.w,
          h: el.position.h,
          sizing: { type: 'cover', w: el.position.w, h: el.position.h }
        });
      } else {
...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    // Extract speaker notes from hidden div or <aside class="notes">
    const notesEl = document.querySelector('.speaker-notes, aside.notes');
    const speakerNotes = notesEl ? notesEl.textContent.trim() : '';

    return { background, elements, placeholders, errors, contrastWarnings, speakerNotes };
  });
}
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    if (slideData.speakerNotes) {
      targetSlide.addNotes(slideData.speakerNotes);
    }
