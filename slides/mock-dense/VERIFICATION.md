...
### 9.1 FN(정탐 놓침) 회귀 — 전수 확인 0건

0.92로 격하된 정상덱(ai+samsung) 19건의 1.0 fills% = **전부 95~101%**(임계 0.95를 막 넘긴 경계). CJK 1.0이 실측 대비 ~8% 과대라 실제론 88~93%(여유). 경계 케이스 렌더 전수 대조:
- 101% "온프레미스"·"숨겨진 병목 변수" → 박스 안 1줄.
- 99% "파운드리 흑자 전환 목표"·"전력망 연결에 최대 7년" → 1줄.
- 98% samsung "DS+DX+SDC+Harman 4개 사업부 포트폴리오"·"삼성전자 종목 카드" → 1줄.
- 95% "세계 최초 GAA 양산 완료"·"4대 하이퍼스케일러 투자 비교" → 1줄.

→ **격하 19건 전부 실제 비-overflow = 전부 FP. 0.92가 놓친 TP(진짜 wrap/overflow) 0건.** 0.92보다 더 낮추면 95% 미만(이미 통과)엔 무효, 100%+ 진짜 wrap을 놓쳐 FN 발생 → 0.92 = FP/FN 균형점.
- **잔여 568건 ≠ 전부 FP**: will overflow 4(진짜 넘침 TP) + may wrap 437(빽빽 한글 실제 wrap 다수=TP) + fits 127. may wrap이 많은 건 빽빽 덱 특성이지 FP 아님.
