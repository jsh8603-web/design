...
export function buildCodexEditPrompt({ slideFile, slidePath, userPrompt, selections = [] }) {
  ...
  return [
    ...
    '- NEVER nest <div> inside <p>, <h1>-<h6>, or <li> — browsers auto-close the parent tag, creating broken orphan nodes.',
    '- For inline styling within text, use <span> (not <div style="display:inline-block">).',
    '- NEVER put <p> inside <h1>-<h6> or another <p>.',
    '- Undo/Revert: If user asks to revert, undo, or restore original, run `git checkout -- <slide-path>` to restore the last committed version.',
    '',
    'Image changes (사내 환경 — AI 이미지 생성 불가):',
    '- AI image generation (Gemini/NanoBanana) is DISABLED in the corporate environment.',
    '- For image change/replace requests: prefer charts/tables/shapes built in HTML/CSS over photos.',
    '- If a photo is required, leave the existing <img> or an empty placeholder and tell the user to',
    '  drop in a manual / internal-asset-library image, then update <img src="..."> to that file.',
    '- Do NOT attempt to call any image-generation script. See design-system/image_slot_contract.md.',
